  ...
  render() {
    ...
    return(
      <View {...other} style={[mainStyles.container, style]} onLayout={this._measureContainer}>
        ...
        >
          {showSmallThumb? this._renderSmallThumbImage() : this._renderThumbImage()}
        </Animated.View>
        <View
          renderToHardwareTextureAndroid={true}
...
//   };
  ...
  _renderThumbImage = () => {
    var {thumbImage, thumbStyle} = this.props;
    if (!thumbImage) return;
    
    return <Image source={thumbImage}/>;
  };
  _renderSmallThumbImage = () => {
     var {thumbImage, thumbStyle} = this.props;
     if (!thumbImage) return;
     
     return <Image source={thumbImage} style={{width:thumbStyle.width, height:thumbStyle.height, borderRadius:thumbStyle.width/2}}/>;
   };
}
